...
(async () => {
  console.debug(`worker.init begin id=${threadId} kind=${workerData.kind} port=${workerData.port ?? 'n/a'}`);
  ...
  console.debug(`worker.load begin id=${threadId} kind=${workerData.kind}`);
  await application.load({ invoke });
  console.debug(`worker.load end id=${threadId} kind=${workerData.kind}`);
