...
class Application extends EventEmitter {
  ...
  async start() {
    ...
    this.console.debug(`app.start begin kind=${kind} port=${port ?? 'n/a'}`);
    if (kind === 'server' || kind === 'balancer') {
      ...
      this.console.debug(`server.listen begin kind=${kind} port=${port}`);
      await this.server.listen();
      this.console.debug(`server.listen end kind=${kind} port=${port}`);
    }
    if (sandbox.api.auth) {
    ...
    this.console.debug(`app.start end kind=${kind} port=${port ?? 'n/a'}`);
