...
class Server {
  ...
  listen() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onError = (err) => {
        console.error(`Listen error: ${err.code || 'UNKNOWN'} ${err.message}`);
        if (err.code !== 'EADDRINUSE') return void reject(err);
        this.retry--;
      ...
      if (this.wsServer) this.wsServer.on('error', onError);

      console.debug(`Listen begin host=${host} port=${port}`);
      this.httpServer.listen(port, host);
    });
  }
